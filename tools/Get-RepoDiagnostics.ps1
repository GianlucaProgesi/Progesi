...
function Resolve-Repo {
  ...
  if (Get-Command git -ErrorAction SilentlyContinue) {
    ...
    if ($LASTEXITCODE -eq 0 -and $url -match '[:/](?<o>[^/]+)/(?<r>[^/\.]+)(?:\.git)?$') { return ($Matches.o + "/" + $Matches.r) }
  }
...
try {
  ...
  $repoJson = gh repo view $Repo --json name,owner,defaultBranchRef,visibility,url | ConvertFrom-Json
  $wfs = gh workflow list --repo $Repo --json id,name,path,state | ConvertFrom-Json
  ...
  foreach ($wf in $wfs) {
    try {
      ...
      foreach ($r in $rr) {
        $perWf += [pscustomobject]@{
          workflow = $wf.name; path = $wf.path; id = $r.databaseId; title = $r.displayTitle
          branch = $r.headBranch; status = $r.status; concl = $r.conclusion; created = $r.createdAt; updated = $r.updatedAt; url = $r.url
        }
      }
    } catch {}
  }
  ...
  if ($failed) {
    $failedReport = gh run view $failed.databaseId --repo $Repo --json databaseId,status,conclusion,startedAt,updatedAt,headSha,headBranch,workflowName,jobs,url | ConvertFrom-Json
  ...
  ([pscustomobject]@{
    repo = $repoJson; workflows = $wfs; recentRuns = $runs; runsPerWorkflow = $perWf; lastFailed = $failed; lastFailedReport = $failedReport
  ...
  ("- URL: " + $repoJson.url) | Add-Content $mdPath
  "" | Add-Content $mdPath

  "## Workflows" | Add-Content $mdPath
  foreach ($wf in $wfs) { ("- " + $wf.name + " — " + $wf.state + " — path: " + $wf.path) | Add-Content $mdPath }
  "" | Add-Content $mdPath
