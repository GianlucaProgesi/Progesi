...
## ProgesiAxisVariable value-curve direction accepted — 2026-08-16 (B3-pre)

Records acceptance of the value-curve ADR — canonical **ADR-019** in the Notion ADR DB (drafted on disk as `ADR-017-DRAFT-AxisVar-value-curve.md`; the ADR-017 number was already assigned, so the DB assigned ADR-019), Track B / B3-pre: ProgesiAxisVariable value variation is modelled as a 2D value-curve (serialized Rhino-free NURBS payload + a pure-managed evaluator), NOT hand-typed analytical expressions. This SUPERSEDES the old Strategy requirement "must support mathematical expressions." Option C evolves ProgesiFunction's piecewise container into ProgesiValueCurve (segment kinds Nurbs / Constant / Undefined), reusing the container, DTO, FunctionRef, hashing hooks, and persistence columns (FunctionPayload holds serialized NURBS → likely no EF migration).

ProgesiCore stays Rhino/EF/UI/ASP.NET-free (§4): the value curve is a serialized payload (like the axis CurvePayload); all Rhino math stays at the adapter. Trade-off recorded: transcendental laws become high-accuracy NURBS approximations (engineering-exact at any real tolerance), in exchange for determinism and culture-invariance.

Retirement of the Expression kind + evaluator + its 22 tests is DEFERRED under ADR-014 (doc + validation + rollback) until the value curve is proven end-to-end at B3c/B3d; the expression path is kept dormant as a fallback until then. This note grants no new authorisation: implementation runs only in 02 Cursor Bridge, each package Red-gated (brief + branch + tests + review + manual GH where GH-coupled). AxisVar's scoped unfreeze (2026-07-30, ADR-008/011) governs; §4 and the §9 guard remain in force. B3b pins the exact variation-component roster.
