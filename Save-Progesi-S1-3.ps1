param(
  ...
  [string]$Body   = @'
VarOut: Hash/Id opzionali (liste/alberi), output tree completi (ValC, MetaId, Depends, Ass, LM, Info). Priorità Hash.
MetaOut: Hash/Id opzionali (liste/alberi), output tree (Id, Hash umano, By, Description, Refs, Snips, LM, Info).
MetaIn: input "Description", summary umano, dedupe BY+Description, salvataggio Refs normalizzati.
Compat RHINO-only: indici VarHash/VarStrictHash + MetaContentHash; niente SQLite/mock. Build 0 warning; Tests 107/107.
'@,
...
if ($current -ne $Branch) {
  ...
  if ($LASTEXITCODE -eq 0) { Invoke-Git @('switch', $Branch) }
  else { Invoke-Git @('switch','-c', $Branch) }
}
...
# 4) push – se non c’è upstream lo crea
try {
  Invoke-Git @('push')
} catch {
