...
<#
Repo-Delete-ByCsv_v2.ps1
- Silenzia i warning di git (stderr) per evitare NativeCommandError in PowerShell.
- Usa --ignore-unmatch su git rm.
- Evita 'git add .' se la working tree non e' pulita: consiglia di committare/stashare prima.
#>
...
if ($inGit) {
  ...
  if ($dirty) { Write-Yellow "Working tree NON pulita. Consigliato commit/stash PRIMA della pulizia." }
}
...
if ($DeleteCsv) {
  ...
  foreach ($r in $rows) {
    ...
    if ($suggest -and ($suggest -like "DELETE*")) {
      ...
      if ($resolved) { $toRemove.Add($resolved) } else { Write-Gray "Non trovato dal CSV: $p" }
    }
  }
}
...
if (-not $targets -or $targets.Count -eq 0) { Write-Host "Niente da eliminare"; Pop-Location; exit 0 }

if (-not $NoConfirm) {
  Write-Host ("Verranno rimossi {0} elementi." -f $targets.Count) -ForegroundColor Cyan
  $answer = Read-Host "Confermi? (Y/N)"
  if ($answer.Trim().ToUpper() -ne 'Y') { Write-Host "Annullato."; Pop-Location; exit 0 }
}
...
if ($GitUntrack -and $inGit) {
  ...
  foreach ($n in $untrack) {
    try {
      & git rm -r --cached --ignore-unmatch $n 2>$null 1>$null
    } catch {}
  }
  # 'git add -A' genera warning CRLF su alcuni file; lo silenziamo e non falliamo
  if (-not $DryRun) {
    try { & git add -A 2>$null 1>$null } catch {}
  }
  Write-Gray "Git index ripulito (se presente)."
}

Write-Green ("Rimozione completata. Removed={0}, Missing={1}, DryRun={2}" -f $removed,$missing,$DryRun)
Pop-Location
